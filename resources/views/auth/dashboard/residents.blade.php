...
<div class="d-flex align-items-center" style="flex: 1; max-width: 300px;">
    <input class="form-control" type="search" id="residentSearch" placeholder="Search residents..." aria-label="Search" style="border-top-right-radius: 0; border-bottom-right-radius: 0;">
    <button class="btn btn-primary" id="searchButton" type="button" style="border-top-left-radius: 0; border-bottom-left-radius: 0;">
    ...
    showAlert(`Resident ${residentId ? 'updated' : 'added'} successfully!`, "success");
            ...
            showAlert(error.message || "Error saving resident", "dan    ger");
        } finally {
...
async function loadResidentForEdit(residentId) {
    ...
    const residentTable = document.querySelector('.table-responsive tbody');
    
    // Function to perform the search
    function performSearch() {
        const searchTerm = searchInput.value.toLowerCase().trim();
        const rows = residentTable.querySelectorAll('tr.clickable-row');
        let hasMatches = false;

        rows.forEach(row => {
            const idText = row.cells[0]?.textContent.toLowerCase().trim() || '';
            const nameText = row.cells[1]?.querySelector('strong')?.textContent.toLowerCase().trim() || '';
        ...
        if (!hasMatches && searchTerm !== '') {
            ...
            `;
            residentTable.appendChild(noResultsRow);
        }
    }
    ...
    // Event listener for the input field to handle Enter key press and input changes
    searchInput.addEventListener('keyup', function(e) {
        ...
        // Automatically clear results if the search is empty
        if (searchInput.value.trim() === '') {
            clearSearch();
        }
    });
